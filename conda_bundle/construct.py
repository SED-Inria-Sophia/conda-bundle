...
from conda_bundle.exceptions import (
...
By default, conda_bundle will error out when adding packages with duplicate
...
whether or not to let the user to register its shell to open the embedded Conda console in any folder of the Windows Explorer.
    '''),
    ## TODO: implement optional_post_install scripts in installer scripts (winexe.py, ospkg.py, shar.py
    ## TODO: I left that so that the key does not raise an error when construct.yaml is parsed.
...
def yamlize(data, directory, content_filter):
    ...
    try:
        ...
        try:
            from conda_bundle.jinja import render_jinja
